...
Windowsではリポジトリ直下の **`preview-local.bat` をダブルクリック**してください（PowerShellを使う場合は `preview-local.ps1` でも可）。ローカルサーバーが起動し、`http://localhost:8000/projects.html` で確認できます。この方法なら、このフォルダにPNGを追加したあとページを再読み込みするだけで反映されます。GitHubへのPushやmanifest更新は不要です。
